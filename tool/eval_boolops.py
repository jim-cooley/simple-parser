...
def eval_boolops_dispatch(node, left, right):
    l_value = left
    ...
    if l_value is None or r_value is None:
        return None
    return eval_boolops_dispatch2(node.token.id, l_value, r_value)


def eval_boolops_dispatch2(tkid, l_value, r_value):
    l_ty = type(l_value).__name__
    r_ty = type(r_value).__name__
    ...
    if l_ty in _type2idx and r_ty in _type2idx:
        ixl = _type2idx[l_ty]
        ixr = _type2idx[r_ty]
        fn = _boolops_dispatch_table[tkid][ixr][ixl]
