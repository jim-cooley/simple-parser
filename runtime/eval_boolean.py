...
from runtime.conversion import c_box, c_to_bool, c_to_float, c_to_int, c_unbox, c_to_dur
from runtime.environment import Environment
from runtime.exceptions import runtime_error
from runtime.pandas import pd_gtr_df, pd_less_df, pd_eq_df, pd_or_df, pd_and_df, pd_neq_df, pd_gte_df, pd_lte_df
...
def _and__dataframe_dataframe(l_value, r_value):
    return pd_and_df(l_value, r_value)
...
def _or__dataframe_dataframe(l_value, r_value):
    return pd_or_df(l_value, r_value)
...
def _iseq__any_dataframe(l_value, r_value):
    return pd_eq_df(l_value, r_value)


def _iseq__dataframe_any(l_value, r_value):
    return pd_eq_df(r_value, l_value)
...
def _neq__any_dataframe(l_value, r_value):
    return pd_neq_df(l_value, r_value)


def _neq__dataframe_any(l_value, r_value):
    return pd_neq_df(r_value, l_value)
...
def _gtr__str_any(l_value, r_value):
    if isinstance(r_value, int) or isinstance(r_value, timedelta):
        l_val = int(l_value)
        return l_val > r_value
...
def _gtr__any_str(l_value, r_value):
    if isinstance(l_value, int) or isinstance(r_value, timedelta):
        r_val = int(r_value)
        return l_value > r_val
    if isinstance(l_value, float):
        r_val = float(r_value)
        return l_value > r_val
    return l_value > r_value


def _gtr__str_str(l_value, r_value):
    return l_value > r_value
...
def _gtr__any_dataframe(l_value, r_value):
    return pd_gtr_df(l_value, r_value)


def _gtr__dataframe_any(l_value, r_value):
    return pd_gtr_df(r_value, l_value)
...
def _less__str_any(l_value, r_value):
    if isinstance(r_value, int) or isinstance(r_value, timedelta):
        l_val = int(l_value)
        return l_val < r_value
    if isinstance(r_value, float):
...
def _less__dataframe_any(l_value, r_value):
    return pd_less_df(r_value, l_value)


def _less__any_dataframe(l_value, r_value):
    return pd_less_df(l_value, r_value)
...
def _gte__str_any(l_value, r_value):
    if isinstance(r_value, int) or isinstance(r_value, timedelta):
        l_val = int(l_value)
        return l_val >= r_value
    if isinstance(r_value, float):
...
def _gte__any_dataframe(l_value, r_value):
    return pd_gte_df(l_value, r_value)


def _gte__dataframe_any(l_value, r_value):
    return pd_gte_df(r_value, l_value)
...
def _lte__str_any(l_value, r_value):
    if isinstance(r_value, int) or isinstance(r_value, timedelta):
        l_val = int(l_value)
        return l_val <= r_value
    if isinstance(r_value, float):
...
def _lte__any_dataframe(l_value, r_value):
    return pd_lte_df(l_value, r_value)


def _lte__dataframe_any(l_value, r_value):
    return pd_lte_df(r_value, l_value)
...
_boolean_dispatch_table = {
    ...
    TK.ISEQ: [
        ...
        [_iseq__any_dataframe, _invalid_iseq,    _iseq__any_dataframe, _iseq__any_dataframe, _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _iseq__any_dataframe, _invalid_iseq,    _iseq__series_dataframe, _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq],   # DataFrame  
        ...
        [_invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _iseq__series_series, _invalid_iseq,    _invalid_iseq,    _invalid_iseq,    _invalid_iseq],   # Series  
    ...
    TK.NEQ: [
        ...
        [_neq__any_dataframe, _invalid_neq,     _neq__any_dataframe, _neq__any_dataframe, _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _neq__any_dataframe, _invalid_neq,     _neq__series_dataframe, _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq],   # DataFrame  
        ...
        [_invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq,     _neq__series_series, _invalid_neq,     _invalid_neq,     _invalid_neq,     _invalid_neq],   # Series  
    ...
    TK.GTR: [
        # any               none              int               float             bool              str               datetime          timedelta         object            block             dataframe         range             series            set               list              ndarray           function          
        [_gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__str_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _invalid_gtr,     _gtr__dataframe_any, _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # any
        [_gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__str_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # none
        [_gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__str_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _invalid_gtr,     _gtr__dataframe_any, _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # int
        [_gtr__any_any,    _gtr__any_any,    _gtr__int_float,  _gtr__int_float,  _gtr__any_any,    _gtr__str_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _invalid_gtr,     _gtr__dataframe_any, _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # float
        [_gtr__any_bool,   _gtr__any_bool,   _gtr__any_bool,   _gtr__float_bool, _gtr__bool_bool,  _gtr__str_any,    _gtr__datetime_bool, _gtr__datetime_bool, _gtr__any_any,    _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # bool
        [_gtr__any_str,    _gtr__any_str,    _gtr__any_str,    _gtr__any_str,    _gtr__any_str,    _gtr__str_str,    _gtr__any_str,    _gtr__any_str,    _gtr__any_str,    _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # str
        [_gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__str_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # datetime
        [_gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _gtr__str_any,    _gtr__any_any,    _gtr__any_any,    _gtr__any_any,    _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # timedelta
        ...
        [_gtr__any_dataframe, _invalid_gtr,     _gtr__any_dataframe, _gtr__any_dataframe, _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _gtr__any_dataframe, _invalid_gtr,     _gtr__series_dataframe, _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # DataFrame  
        ...
        [_invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _gtr__series_series, _invalid_gtr,     _invalid_gtr,     _invalid_gtr,     _invalid_gtr],   # Series  
    ...
    TK.LESS: [
        # any               none              int               float             bool              str               datetime          timedelta         object            block             dataframe         range             series            set               list              ndarray           function          
        [_less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__str_any,   _less__any_any,   _less__any_any,   _less__any_any,   _invalid_less,    _less__dataframe_any, _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less],   # any
        [_less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__str_any,   _less__any_any,   _less__any_any,   _less__any_any,   _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less],   # none
        [_less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__str_any,   _less__any_any,   _less__any_any,   _less__any_any,   _invalid_less,    _less__dataframe_any, _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less],   # int
        [_less__any_any,   _less__any_any,   _less__int_float, _less__int_float, _less__any_any,   _less__str_any,   _less__any_any,   _less__any_any,   _less__any_any,   _invalid_less,    _less__dataframe_any, _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less],   # float
        [_less__any_bool,  _less__any_bool,  _less__any_bool,  _less__float_bool, _less__bool_bool, _less__str_any,   _less__datetime_bool, _less__datetime_bool, _less__any_any,   _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less],   # bool
        [_less__any_str,   _less__any_str,   _less__any_str,   _less__any_str,   _less__any_str,   _less__any_any,   _less__any_str,   _less__any_str,   _less__any_str,   _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less],   # str
        [_less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__str_any,   _less__any_any,   _less__any_any,   _less__any_any,   _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less],   # datetime
        [_less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__any_any,   _less__str_any,   _less__any_any,   _less__any_any,   _less__any_any,   _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less,    _invalid_less],   # timedelta
    ...
    TK.GTE: [
        # any               none              int               float             bool              str               datetime          timedelta         object            block             dataframe         range             series            set               list              ndarray           function          
        [_gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__str_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _invalid_gte,     _gte__dataframe_any, _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte],   # any
        [_gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__str_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte],   # none
        [_gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__str_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _invalid_gte,     _gte__dataframe_any, _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte],   # int
        [_gte__any_any,    _gte__any_any,    _gte__int_float,  _gte__int_float,  _gte__any_any,    _gte__str_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _invalid_gte,     _gte__dataframe_any, _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte],   # float
        [_gte__any_bool,   _gte__any_bool,   _gte__any_bool,   _gte__float_bool, _gte__bool_bool,  _gte__str_any,    _gte__datetime_bool, _gte__datetime_bool, _gte__any_any,    _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte],   # bool
        [_gte__any_str,    _gte__any_str,    _gte__any_str,    _gte__any_str,    _gte__any_str,    _gte__any_any,    _gte__any_str,    _gte__any_str,    _gte__any_str,    _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte],   # str
        [_gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__str_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte],   # datetime
        [_gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _gte__str_any,    _gte__any_any,    _gte__any_any,    _gte__any_any,    _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte,     _invalid_gte],   # timedelta
    ...
    TK.LTE: [
        # any               none              int               float             bool              str               datetime          timedelta         object            block             dataframe         range             series            set               list              ndarray           function          
        [_lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__str_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _invalid_lte,     _lte__dataframe_any, _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte],   # any
        [_lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__str_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte],   # none
        [_lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__str_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _invalid_lte,     _lte__dataframe_any, _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte],   # int
        [_lte__any_any,    _lte__any_any,    _lte__int_float,  _lte__int_float,  _lte__any_any,    _lte__str_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _invalid_lte,     _lte__dataframe_any, _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte],   # float
        [_lte__any_bool,   _lte__any_bool,   _lte__any_bool,   _lte__float_bool, _lte__bool_bool,  _lte__str_any,    _lte__datetime_bool, _lte__datetime_bool, _lte__any_any,    _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte],   # bool
        [_lte__any_str,    _lte__any_str,    _lte__any_str,    _lte__any_str,    _lte__any_str,    _lte__any_any,    _lte__any_str,    _lte__any_str,    _lte__any_str,    _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte],   # str
        [_lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__str_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte],   # datetime
        [_lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _lte__str_any,    _lte__any_any,    _lte__any_any,    _lte__any_any,    _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte,     _invalid_lte],   # timedelta
